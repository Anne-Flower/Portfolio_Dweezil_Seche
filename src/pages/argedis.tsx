...
const Argedis = () => {
  ...
  return (
    <>
      ...
      >
        <div
          className={`relative flex justify-center font-extralight w-screen flex-col min-h-screen overflow-clip bg-beige ${
        ...
        >
          ...
          >
            <div className="flex flex-col md:flex-row text-black w-screen md:pl-[160px] w-[345px] md:w-full ">
              ...
              <div className="flex flex-col h-[394px] md:h-[245px] w-[345px] md:w-[415px] ml-6 md:ml-52 ">
                <p className="menca md:w-[415px] md:pt-52 pt-4  font-medium	text-base	leading-5	">
                  Regionalisme is an application designed for use on interactive
                  kiosks at petrol stations all over France. It&apos;s a project
                  I carried out during my work-study year at Studio Artefact
                  3000. The aim of this application is to promote local
                  producers who supply service stations with their regional
                  products, and to encourage customers to buy local products
                  rather than Lays chips.
                </p>
                <p className="menca md:w-[415px] pt-4 font-medium	text-base	leading-5">
                  My role on this project was to adapt existing prints displayed
                  in service stations to a digital format. We worked on nearly a
                  hundred service stations, each with its own color code and
                  specific producers. We then animated all the little
                  illustrations decorating the maps.
                </p>
              </div>
            </div>
          ...
          <div className="md:visible invisible pt-0 md:pt-48 bg-beige">
            <motion.div
              className="h-0 md:h-[900px] bg-beige "
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              transition={{ duration: 0.6 }}
              viewport={{ once: false }}
            >
              <img
                src="/assets/imagesV2/argedis/img_argedis.png"
                alt=""
                className="md:h-[900px] bg-beige w-screen"
              />
            </motion.div>
          </div>
          <div id="img_regiolnalisme" className="bg-beige md:pt-[40px] pt-2 ">
            {images_regionalisme.map((img, index) => (
              <img
                key={index}
                src={img.src}
                alt={`Regionalisme ${index}`}
                className="w-full h-[184px] md:h-[727px] px-6 md:px-10 py-2 md:py-6"
              />
            ))}
            <img
              src="/assets/imagesV2/argedis/presentation_argedis.png"
              alt="présentation argedis"
              className="w-full h-[215px] md:h-[850px]  py-2 md:py-6"
            />
          </div>
          <div className="bg-beige w-full h-[310px] md:h-[927px] px-6 md:px-10 pt-5 py-2 md:py-6">
          ...
          <div className="pb-[64px] bg-beige text-black md:pt-[340px]">
            <BarDecormate
              label={"( UI )"}
              label2={"( DA )"}
              label3={"( branding )"}
